...
const NAN_BUFFER = byte_utils_1.ByteUtils.fromNumberArray([
    0x7c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
].reverse());
// Infinity value bits 32 bit values (due to lack of longs)
const INF_NEGATIVE_BUFFER = byte_utils_1.ByteUtils.fromNumberArray([
    0xf8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
].reverse());
const INF_POSITIVE_BUFFER = byte_utils_1.ByteUtils.fromNumberArray([
...
function invalidErr(string, message) {
    throw new error_1.BSONError(`"${string}" is not a valid Decimal128 string - ${message}`);
}
/**
 ...
 * @category BSONType
 */
class Decimal128 extends bson_value_1.BSONValue {
    get _bsontype() {
        return 'Decimal128';
    }
    /**
    ...
    constructor(bytes) {
        ...
        else if (bytes instanceof Uint8Array || (0, utils_1.isUint8Array)(bytes)) {
            if (bytes.byteLength !== 16) {
                throw new error_1.BSONError('Decimal128 must take a Buffer of 16 bytes');
            }
            this.bytes = bytes;
        }
        else {
            throw new error_1.BSONError('Decimal128 must take a Buffer or string');
        }
    }
    /**
    ...
    static _fromString(representation, options) {
        ...
        if (representation.length >= 7000) {
            throw new error_1.BSONError('' + representation + ' not a valid Decimal128 string');
        }
        // Results
        ...
        const buffer = byte_utils_1.ByteUtils.allocateUnsafe(16);
        index = 0;
