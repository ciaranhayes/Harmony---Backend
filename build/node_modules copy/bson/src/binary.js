...
exports.validateBinaryVector = validateBinaryVector;
 ...
 * @category BSONType
 */
class Binary extends bson_value_1.BSONValue {
    get _bsontype() {
        return 'Binary';
    }
    /**
    ...
    constructor(buffer, subType) {
        ...
        if (!(buffer == null) &&
            ...
            !(0, utils_1.isAnyArrayBuffer)(buffer) &&
        ...
        if (buffer == null) {
            ...
            this.buffer = byte_utils_1.ByteUtils.allocate(Binary.BUFFER_SIZE);
            this.position = 0;
        }
        else {
    ...
    put(byteValue) {
        ...
        if (this.buffer.byteLength > this.position) {
            ...
            const newSpace = byte_utils_1.ByteUtils.allocate(Binary.BUFFER_SIZE + this.buffer.length);
    ...
    write(sequence, offset) {
        ...
        if (this.buffer.byteLength < offset + sequence.length) {
            const newSpace = byte_utils_1.ByteUtils.allocate(this.buffer.byteLength + sequence.length);
            newSpace.set(this.buffer, 0);
    ...
    toExtendedJSON(options) {
        ...
        const base64String = byte_utils_1.ByteUtils.toBase64(this.buffer);
    ...
    inspect(depth, options, inspect) {
        ...
        const base64 = byte_utils_1.ByteUtils.toBase64(this.buffer.subarray(0, this.position));
    ...
    toInt8Array() {
        if (this.sub_type !== Binary.SUBTYPE_VECTOR) {
            throw new error_1.BSONError('Binary sub_type is not Vector');
        }
        if (this.buffer[0] !== Binary.VECTOR_TYPE.Int8) {
            throw new error_1.BSONError('Binary datatype field is not Int8');
        }
    ...
    toFloat32Array() {
        ...
        if (this.buffer[0] !== Binary.VECTOR_TYPE.Float32) {
            throw new error_1.BSONError('Binary datatype field is not Float32');
        }
        const floatBytes = new Uint8Array(this.buffer.buffer.slice(this.buffer.byteOffset + 2, this.buffer.byteOffset + this.position));
        if (number_utils_1.NumberUtils.isBigEndian)
            byte_utils_1.ByteUtils.swap32(floatBytes);
    ...
    toPackedBits() {
        ...
        if (this.buffer[0] !== Binary.VECTOR_TYPE.PackedBit) {
            throw new error_1.BSONError('Binary datatype field is not packed bit');
        }
    ...
    static fromInt8Array(array) {
        const buffer = byte_utils_1.ByteUtils.allocate(array.byteLength + 2);
    ...
    static fromFloat32Array(array) {
        const binaryBytes = byte_utils_1.ByteUtils.allocate(array.byteLength + 2);
        binaryBytes[0] = Binary.VECTOR_TYPE.Float32;
        binaryBytes[1] = 0;
        const floatBytes = new Uint8Array(array.buffer, array.byteOffset, array.byteLength);
        binaryBytes.set(floatBytes, 2);
        if (number_utils_1.NumberUtils.isBigEndian)
            byte_utils_1.ByteUtils.swap32(new Uint8Array(binaryBytes.buffer, 2));
    ...
    static fromBits(bits) {
        ...
        for (let bitOffset = 0; bitOffset < bits.length; bitOffset++) {
            ...
            if (bit !== 0 && bit !== 1) {
                throw new error_1.BSONError(`Invalid bit value at ${bitOffset}: must be 0 or 1, found ${bits[bitOffset]}`);
            }
            if (bit === 0)
                continue;
            const shift = 7 - (bitOffset % 8);
...
function validateBinaryVector(vector) {
    ...
    if ((datatype === Binary.VECTOR_TYPE.Float32 || datatype === Binary.VECTOR_TYPE.Int8) &&
        padding !== 0) {
        throw new error_1.BSONError('Invalid Vector: padding must be zero for int8 and float32 vectors');
    }
    if (datatype === Binary.VECTOR_TYPE.PackedBit && padding !== 0 && size === 2) {
        throw new error_1.BSONError('Invalid Vector: padding must be zero for packed bit vectors that are empty');
    }
    if (datatype === Binary.VECTOR_TYPE.PackedBit && padding > 7) {
        throw new error_1.BSONError(`Invalid Vector: padding must be a value between 0 and 7. found: ${padding}`);
    }
}
...
class UUID extends Binary {
    ...
    constructor(input) {
        ...
        super(bytes, constants_1.BSON_BINARY_SUBTYPE_UUID_NEW);
    }
    /**
     * The UUID bytes
     * @readonly
     ...
     */
    static generate() {
        const bytes = byte_utils_1.ByteUtils.randomBytes(UUID_BYTE_LENGTH);
