    ...
    execute(connection, credentials) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.cache.hasRefreshToken) {
                ...
                const result = yield this.fetchAccessToken(this.cache.getIdpInfo(), credentials, refreshToken);
                this.cache.put(result);
                connection.accessToken = result.accessToken;
                try {
                    return yield this.finishAuthentication(connection, credentials, result.accessToken);
                }
                catch (error) {
                    if (error instanceof error_1.MongoError &&
                        error.code === error_1.MONGODB_ERROR_CODES.AuthenticationFailed) {
            ...
            const startResponse = yield this.startAuthentication(connection, credentials);
            const conversationId = startResponse.conversationId;
            const idpInfo = bson_1.BSON.deserialize(startResponse.payload.buffer);
            const callbackResponse = yield this.fetchAccessToken(idpInfo, credentials);
            this.cache.put(callbackResponse, idpInfo);
            connection.accessToken = callbackResponse.accessToken;
            return yield this.finishAuthentication(connection, credentials, callbackResponse.accessToken, conversationId);
        });
    }
    /**
     * Fetches an access token using the callback.
     */
    fetchAccessToken(idpInfo, credentials, refreshToken) {
        return __awaiter(this, void 0, void 0, function* () {
