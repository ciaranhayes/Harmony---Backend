...
class AWSTemporaryCredentialProvider {
    static get awsSDK() {
        var _a;
        (_a = AWSTemporaryCredentialProvider._awsSDK) !== null && _a !== void 0 ? _a : (AWSTemporaryCredentialProvider._awsSDK = (0, deps_1.getAwsCredentialProvider)());
...
class AWSSDKCredentialProvider extends AWSTemporaryCredentialProvider {
    ...
    getCredentials() {
        return __awaiter(this, void 0, void 0, function* () {
             ...
             */
            try {
                const creds = yield this.provider();
                return {
    ...
    getCredentials() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (process.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI) {
                return yield (0, utils_1.request)(`${AWS_RELATIVE_URI}${process.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI}`);
            }
            // Otherwise assume we are on an EC2 instance
            // get a token
            const token = yield (0, utils_1.request)(`${AWS_EC2_URI}/latest/api/token`, {
                method: 'PUT',
                json: false,
                headers: { 'X-aws-ec2-metadata-token-ttl-seconds': 30 }
            });
            // get role name
            const roleName = yield (0, utils_1.request)(`${AWS_EC2_URI}/${AWS_EC2_PATH}`, {
                json: false,
                headers: { 'X-aws-ec2-metadata-token': token }
            });
            // get temp credentials
            const creds = yield (0, utils_1.request)(`${AWS_EC2_URI}/${AWS_EC2_PATH}/${roleName}`, {
                headers: { 'X-aws-ec2-metadata-token': token }
            });
            return creds;
        });
